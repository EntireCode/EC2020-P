...
namespace EC.Web.ViewComponents.Base
{
    public class SidebarNavMenuViewComponent : ViewComponent
    {
        ...
        public SidebarNavMenuViewComponent(
        ...
        public async Task<IViewComponentResult> InvokeAsync(string filter)
        {
            ...
            if (filter == "Security")
            {
                foreach (var menu in assignedMenus.Where(x => x.MenuCaption == filter).OrderBy(x => x.Position).ToList())
                {
            ...
            {
                foreach (var menu in assignedMenus.Where(x => x.Parent != null && x.Parent.Name == filter).OrderBy(x => x.Position).ToList())
                {
                    if (!menus.Exists(x => x.Name == menu.Name))
                    {
                        menus.Add(menu);
            ...
            return View("SidebarNavMenu", menuContent);
        }
