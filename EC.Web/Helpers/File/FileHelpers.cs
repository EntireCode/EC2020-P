...
namespace EC.Web.Helpers.File
{
    public class FileHelpers
    {
        public static async Task<string> ProcessFormFile(IFormFile formFile,
            ModelStateDictionary modelState, List<string> contentTypes)
        {
            ...
            // a display name.
            MemberInfo property = typeof(FileUploadViewModel).GetProperty(
            ...
            if (!contentTypes.Contains(formFile.ContentType.ToLower()))
            {
                var allowedContentTypes = string.Join(",", contentTypes);

                modelState.AddModelError(formFile.Name, $"The ContentType of {fieldDisplayName}file ({fileName}) must be in [{allowedContentTypes}].");
            }
                        ...
                        StreamReader reader =
                            new StreamReader(
                                ...
                                new UTF8Encoding(encoderShouldEmitUTF8Identifier: false,
                                    throwOnInvalidBytes: false), //change throwOnInvalidBytes to false due to image file will throw Invalid Bytes
